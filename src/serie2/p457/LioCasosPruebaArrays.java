 ...
 *DA UN TLE
 */
public class LioCasosPruebaArrays {
	...
	public static void main(String[] args) {
		int[] listaCasos;  //array estático donde guardamos la lista de casos
		int numCasos;
		int perdido;
		int salto;
		int indice;
		...
		boolean terminar=false;   //condición de terminación 
			...
			while  (!terminar && lector.hasNext()) {
				...
				if (numCasos>0) {
					// procesamiento
					//agregamos todos los casos en un array que instanciamos previamente
					listaCasos= new int[numCasos];
					
					for (int k=0; k<numCasos; k++) {
						//leemos un nuevo valorvalor y lo agregamos a la lista
						listaCasos[k]=lector.nextInt();
					}
					/*
